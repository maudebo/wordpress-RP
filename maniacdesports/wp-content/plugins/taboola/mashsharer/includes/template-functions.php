...
function mashsb_subscribe_content() {
	global $mashsb_options;
	if (isset($mashsb_options['networks'][2]) && isset($mashsb_options['subscribe_behavior']) && $mashsb_options['subscribe_behavior'] === 'content') {
...
function arrNetworks($name, $is_shortcode) {
	...
	$networks_arr = array(
		'partager sur facebook' => 'http://www.facebook.com/sharer.php?u=' . $url,
...
function mashsb_get_twitter_url() {
	...
	} else if (function_exists('mashsuGetShortURL')) {
