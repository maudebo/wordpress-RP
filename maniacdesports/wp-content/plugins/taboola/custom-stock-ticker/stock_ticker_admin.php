...
define(NS.'SP_CURRENT_VERSION', '2.2');   //NOTE: should always match Version: ### in the plugin special comment
define(NS.'SP_TYPE', 'ticker');
define(NS.'DATA_ITEM_COUNT', 6);
...
function stock_ticker_activate() {   
    ...
    if (!get_option('stock_ticker_version')) {
        // if this doesn't exist, assume initial install
        ...
        $values = array( //NOTE: the rest should all be the defaults
                        ...
                        'stock_list'     => '^GSPC,^IXIC,^NYA,MMM,AXP,T,BA,CAT,CVX,CSCO,KO,DD,XOM,GE,GS,HD,INTC,IBM,JNJ,JPM,MCD,MRK,MSFT,NKE,PFE,PG,TRV,UNH,UTX,VZ,V,WMT,DIS',
                        'advanced_style' => 'margin: auto;'
                        );
        sp_add_row($values); //TODO: what happens if this errors for some reason? MUST alert user somehow
        add_option('stock_ticker_version',                               $current_version);
        add_option('stock_ticker_notice_text', "Stock Ticker Installed v{$current_version}");
    }
}
...
function stock_ticker_handle_update() {
    ...
    //NOTE: controls start points in the case block                 //nothing to do
    if     (version_compare($db_version, $current_version, '==')) return;
                                                                    //switched from a version upgrade notice to a generic alert text field
    if     (version_compare($db_version, '2.2', '<'))             $opperation = 'notice_text';
                                                                    //only need to update the version number
    elseif (version_compare($db_version, $current_version,  '<')) $opperation = 'update_latest';
    // NOTE: implied if $db_version > $current_version  we have a downgrade
    
    //NOTE: be careful when editing ANY function called by this upgrade section. Don't want to break the upgrade chain
    switch($opperation) {
        case 'notice_text':
            $tmp = get_option('stock_ticker_per_category_stock_lists');
            sp_update_row(array('stock_list' => $tmp['default']), array('name' => 'Default Settings'));
            delete_option('stock_ticker_per_category_stock_lists');
            delete_option('stock_ticker_version_text'); //NOTE: since this is temporary, we don't need to preserve the old value whatever it was
        case 'update_latest':
            //NOTE: update_option will add the option if it doesnot exist
            update_option('stock_ticker_version',      $current_version);
            update_option('stock_ticker_notice_text', "Stock Ticker updated from v{$db_version} to v{$current_version}"); //TODO: report errors that ocurred during upgrade process
            break;
        case 'newer': //downgrade of some sort, same as default action
        default:
            error_log("DB version missmatch, found old: '$db_version' new: '$current_version'");
            update_option('stock_ticker_notice_text', " found v{$db_version} current version"); //TODO: disable stock tickers until this is resolved???
            break;
    }
}
//*************************************************************************
...
function stock_ticker_admin_enqueue($hook) {
    ...
    //NOTE: must be included on all pages because of dismissible notifications --TODO: tie it into the default ajax wordpress code somehow?
...
function stock_ticker_add_screen_options() {
    ...
    if ($action == 'copy' || $action == 'edit') return; // NOTE: do NOT display screen options on the edit screen since list table is NOT present
...
function stock_ticker_list_page() {
    ...
    switch ($action) {
        case 'copy': //fall through to edit page after copy is complete
        ...
        default:
            ...
            <div id='sp-list-table-page' class='wrap'>
              <h1 style="display:none;">&nbsp;</h1> <!-- notices are auto added here after the first h1/2 element inside .wrap -->
              ...
              <div class="wrap">
HEREDOC;
            //NOTE: extra wrap so that this h2 formatting will be proper
...
HEREDOC;
    ...
    stock_plugin_section_helper(1, 'Ticker Config',        stock_ticker_create_ticker_config( $shortcode_settings));
    stock_plugin_section_helper(2, 'Text Config',          stock_ticker_create_text_config(   $shortcode_settings));
    stock_plugin_section_helper(3, 'Stock Display Config', stock_ticker_create_display_config($shortcode_settings));
    stock_plugin_section_helper(4, 'Advanced Styling',     stock_ticker_create_style_field(   $shortcode_settings));
    echo "
                    </div><!-- end inside -->
                </div><!-- end referrers -->
...
function stock_ticker_create_text_config($shortcode_settings) {
    ...
    foreach($default_fonts as $font) { $tmp .= "<option value='{$font}'></option>\n"; }
...
function stock_ticker_create_style_field($shortcode_settings) {
    return "<p>If you have additional CSS rules you want to apply to the entire ticker (such as alignment or borders) you can add them below.</p>
